...
//	private boolean mShouldCallbackOnUnselectedItemClick = true;
    ...
    public Carousel(Context context, AttributeSet attrs, int defStyle) {
        ...
        // Retrieve settings
        arr = getContext().obtainStyledAttributes(attrs, R.styleable.Carousel);
        ...
        selectedItem = arr.getInteger(R.styleable.Carousel_SelectedItem, 0);
...
//        if (namesForItems != -1) names = getResources().obtainTypedArray(namesForItems);
    ...
    //设置数据源
    public ImageAdapter setAdapter(List<Wallet.Deal> list) {
...
//            System.out.println("the mLastFlingAngle is:" + mLastFlingAngle);
    ...
    private class ImageAdapter extends BaseAdapter {
        ...
        public void SetImages(List<Wallet.Deal> list) {
            SetImages(list, false);
        }

        public void SetImages(List<Wallet.Deal> list, boolean reflected) {

            final int reflectionGap = 4;

            mImages = new CarouselItem[list.size()];

            for (int i = 0; i < list.size(); i++) {
                CarouselItem item = new CarouselItem(mContext);
                item.setIndex(i);
                item.setImageUrl(list.get(i).goodsIco);
                item.setIncoming(list.get(i).subscribeIncome);
                item.setCode(list.get(i).dividendsNum);
                mImages[i] = item;
            }
    ...
    private void Calculate3DPosition(CarouselItem child, int diameter, float angleOffset) {
        ...
        float x = -(float) (diameter / 2 * Math.sin(angleOffset)) * 4 / 5 + diameter / 2
                - child.getWidth() / 2;
//        float z = (float) (diameter / 2 * (1.0f - Math.abs((float) android.util.FloatMath.cos(angleOffset))));
